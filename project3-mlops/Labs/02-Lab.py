...
with mlflow.start_run(run_name= "RF-Grid-Search") as run:
  ...
  rf = RandomForestRegressor(n_estimators=best_rf.get_params()["n_estimators"], max_depth=best_rf.get_params()["max_depth"])
...
print(model_params.get("n_estimators")==best_params.get("n_estimators") and model_params.get("max_depth")==best_params.get("max_depth"))
...
# TODO
# dictionary containing hyperparameter names and list of values we want to try
new_parameters = {'n_estimators': [100, 500, 1000],
              'max_depth': [5, 10, 15],
              'min_samples_split': [1.0, 3.0, 10.0],
              'bootstrap': [True, False]}

rf = RandomForestRegressor()
grid_rf_model = GridSearchCV(rf, new_parameters, cv=3)
grid_rf_model.fit(X_train, y_train)

best_rf2 = grid_rf_model.best_estimator_
for p in new_parameters:
  print("Best '{}': {}".format(p, best_rf2.get_params()[p]))

# COMMAND ----------

best_params2 = best_rf2.get_params()
best_params2
with mlflow.start_run(run_name= "RF-Grid-Search2") as run:
  # Create predictions of X_test using best model
  rf = RandomForestRegressor(n_estimators=best_params2.get("n_estimators"), max_depth=best_params2.get("max_depth"),
                             min_samples_split=best_params2.get("min_samples_split"),bootstrap=best_params2.get("bootstrap"))
  rf.fit(X_train, y_train)
  predictions = rf.predict(X_test)
