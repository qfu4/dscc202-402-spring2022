...
rf2_path = artifactURI + "/random-forest-model-preprocess/"
...
class RF_with_postprocess(mlflow.pyfunc.PythonModel):
    ...
    def postprocess_result(self, results):
        ...
        result_df = pd.DataFrame(results, columns = ["results"])
        bins = [0, 100, np.inf]
        labels = ["Not Expensive","Expensive"]
        result_df['label'] = pd.cut(result_df["results"], bins, labels=labels)
        result_array = result_df['label'].to_numpy()
        return result_array
